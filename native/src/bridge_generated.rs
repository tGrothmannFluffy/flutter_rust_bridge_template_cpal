...
fn wire_get_audio_device_name_impl(port_: MessagePort) {
    FLUTTER_RUST_BRIDGE_HANDLER.wrap(
        WrapInfo {
            debug_name: "get_audio_device_name",
            port: Some(port_),
            mode: FfiCallMode::Normal,
        },
        move || move |task_callback| Ok(get_audio_device_name()),
    )
}
fn wire_start_audio_impl(port_: MessagePort) {
    FLUTTER_RUST_BRIDGE_HANDLER.wrap(
        WrapInfo {
        ...
        move || move |task_callback| Ok(start_audio()),
    )
}
